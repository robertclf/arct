    ...
    if (!i.TweenLite) {
        ...
        r._init = function() {
            ...
            if (r.startAt) {
                ...
                else { i = {}; for (s in r) G[s] && "autoCSS" !== s || (i[s] = r[s]); if (i.overwrite = 0, i.data = "isFromStart", i.lazy = l && r.lazy !== !1, i.immediateRender = l, this._startAt = I.to(this.target, 0, i), l) { if (0 === this._time) return } else this._startAt._init(), this._startAt._enabled(!1) } if (this._ease = h = h ? h instanceof y ? h : "function" == typeof h ? new y(h, r.easeParams) : w[h] || I.defaultEase : I.defaultEase, r.easeParams instanceof Array && h.config && (this._ease = h.config.apply(h, r.easeParams)), this._easeType = this._ease._type, this._easePower = this._ease._power, this._firstPT = null, this._targets)
...
! function() {
    ...
    window.requestAnimationFrame || (window.requestAnimationFrame = function(n) {
        var t = (new Date).getTime(),
            i = Math.max(0, 16 - (t - e)),
            o = window.setTimeout(function() { n(t + i) }, i);
        return e = t + i, o
    }), window.cancelAnimationFrame || (window.cancelAnimationFrame = function(e) { clearTimeout(e) })
}(),
function() {
    function e() {
        s = window.innerWidth, u = window.innerHeight, g = { x: s / 2, y: u / 2 }, w = document.getElementById("large-header"), w.style.height = u + "px", f = document.getElementById("demo-canvas"), f.width = s, f.height = u, h = f.getContext("2d"), v = [];
    ...
    function n() { "ontouchstart" in window || window.addEventListener("mousemove", t), window.addEventListener("scroll", i), window.addEventListener("resize", o) }
    ...
    function i() { y = document.body.scrollTop > u ? !1 : !0 }

    function o() { s = window.innerWidth, u = window.innerHeight, w.style.height = u + "px", f.width = s, f.height = u }

    function a() { r(); for (var e in v) c(v[e]) }

    function r() { 
        if (y) { 
            h.clearRect(0, 0, s, u); 
            // for (var e in v) Math.abs(m(g, v[e])) < 4e3 ? (v[e].active = .3, v[e].circle.active = .6) : Math.abs(m(g, v[e])) < 2e3 ? (v[e].active = .1, v[e].circle.active = .3) : Math.abs(m(g, v[e])) < 4e2 ? (v[e].active = .02, v[e].circle.active = .1) : (v[e].active = 0, v[e].circle.active = 0), l(v[e]), v[e].circle.draw() 
            for (var e in v) Math.abs(m(g, v[e])) < 4e3 ? (v[e].active = .3, v[e].circle.active = .6) : Math.abs(m(g, v[e])) < 2e3 ? (v[e].active = .1, v[e].circle.active = .3) : Math.abs(m(g, v[e])) < 4e2 ? (v[e].active = .02, v[e].circle.active = .1) : (v[e].active = 0, v[e].circle.active = 0), l(v[e]), v[e].circle.draw() 
        } 
        requestAnimationFrame(r) 
    }

    function c(e) { TweenLite.to(e, 1 + 1 * Math.random(), { x: e.originX - 50 + 100 * Math.random(), y: e.originY - 50 + 100 * Math.random(), ease: Circ.easeInOut, onComplete: function() { c(e) } }) }
    ...
    function d(e, n, t) { var i = this;! function() { i.pos = e || null, i.radius = n || null, i.color = t || null }(), this.draw = function() { i.active && (h.beginPath(), h.arc(i.pos.x, i.pos.y, i.radius, 0, 2 * Math.PI, !1), h.fillStyle = "rgba(255,255,255," + i.active + ")", h.fill()) } }

    function m(e, n) { return Math.pow(e.x - n.x, 2) + Math.pow(e.y - n.y, 2) }
